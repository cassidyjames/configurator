...
namespace configurator.Widgets.Fields {
	public class StringInput : Gtk.Entry {
		public StringInput () {
			this.changed.connect (() => {
				var color = Gdk.RGBA ();

				if (color.parse (this.text)) {
					this.override_color (Gtk.StateFlags.NORMAL, color);
				} else {
					this.override_color (Gtk.StateFlags.NORMAL, null);
				}
			});
		}
	}
